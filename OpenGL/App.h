...
#define AppInstance App::getInstance()
...
public:
    ...
    {
        assert(AppInstance._ResizeCallBack);
        glViewport(0, 0, width, height);
        AppInstance._ResizeCallBack(width, height);
    }
    static void keyCallBack(GLFWwindow* window, int key, int scancode, int action, int mods)
    {
        assert(AppInstance._KeyCallBack);
        AppInstance._KeyCallBack(key, scancode, action, mods);
    }
    static void mouseCallBack(GLFWwindow* window, int button, int action, int mods)
    {
        assert(AppInstance._MouseCallBack);
        AppInstance._MouseCallBack(button, action, mods);
    }
    static void cursorCallBack(GLFWwindow* window, double xpos, double ypos)
    {
        assert(AppInstance._CursorCallBack);
        AppInstance._CursorCallBack(xpos, ypos);
    }
    static void scrollCallBack(GLFWwindow* window, double xoffset, double yoffset)
    {
        assert(AppInstance._ScrollCallBack);
        AppInstance._ScrollCallBack(yoffset);
    }
};
